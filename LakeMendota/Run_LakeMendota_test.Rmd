...
5. Prepare and run calibration -- single-model LHC, optional DE refinement,
   writing best parameters back, and calibrating all coupled models at once
   (sequentially or concurrently) with `cali_ensemble_wq()`.
6. Run sensitivity analysis.
7. Compare and visualize model outputs against each other and observations.
...
```{r }
# Set working directory to the project folder that includes all standart input files.
...
## 4) Compute metrics and statistics - this step can be done after calibration to calculate the CSPS metrics.
...
# Plot the best parameters (main config folder)
...
The functions below all plug directly into outputs you've already produced
...
from a NetCDF file (see 7.3) instead of a `cal_metrics()` list -- useful when
you want to compare a variable that wasn't included in `Output.yaml`.
